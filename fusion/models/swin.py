"""Swin Transformer encoder and decoder primitives."""
...
class Encoder(nn.Module):
    def __init__(self, in_ch, feat_chs=[64,128,256,512]):
        super().__init__()
        self.blocks = nn.ModuleList()
        self.pools = nn.ModuleList()
        prev = in_ch
        for ch in feat_chs:
            self.blocks.append(ResidualBlock(prev, ch))
...
#     """FiLM-style modulation per prompt type"""
#     def __init__(self, n_prompts=4, prompt_dim=32, max_channels=384):
#         super().__init__()
#         self.embeddings = nn.Embedding(n_prompts, prompt_dim)
#         self.mlp = nn.Sequential(
#             nn.Linear(prompt_dim, prompt_dim),
#             nn.ReLU(),
#         )
#         self.scale_shift = nn.Linear(prompt_dim, 2 * max_channels)  # 固定参数层
#
...
#         """
#         B, L, C = x.shape
#         emb = self.embeddings(prompt_type)       # [B, prompt_dim]
#         params = self.mlp(emb)                   # [B, prompt_dim]
#         params = self.scale_shift(params)        # [B, 2*max_channels]
#         params = params[:, :2*C]                 # 截取对应通道数量
#         scale, shift = params[:, :C], params[:, C:]
